"""Example of the MBLGF solver.

This solver diagonalises the self-energy via conservation of the spectral moments of the resulting
Green's function, using recursion relations of those moments. The resulting Green's function is
approximate, and is systematically improved by increasing the number of moments (maximum algorithm
...
# Solve the Hamiltonian using the MBLGF solver, initialisation via either:

# 1) Create the solver from the expression
solver = MBLGF.from_expression(exp, max_cycle=1)
solver.kernel()

# 2) Create the solver from a self-energy
solver = MBLGF.from_self_energy(static, self_energy, overlap=overlap, max_cycle=1)
solver.kernel()
