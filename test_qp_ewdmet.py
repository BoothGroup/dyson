...
nmom_max_fci = 3  # Maximum moment order to used for MBLGF
nmom_max_bath = 1  # Maximum EwDMET bath order
nfrag = 2  # Number of sites per fragment
tol = 1e-10  # Threshold for DMET bath
max_cycles = 20  # Maximum number of iterations
conv_tol = 1e-8  # Convergence threshold in the total energy
diis_space = 10  # Size of the DIIS space
eta = 1e-2  # Broadening parameter for the static potential
...
v = np.zeros((nsite, nsite))  # (site|site)
...
for cycle in range(1, max_cycles+1):
    ...
    print("Tr(DM) = {:.8f}".format(np.trace(dm)))

    # Build the DMET bath orbitals
    dm_env = np.linalg.multi_dot((c_env.T, dm, c_env))  # (env|env)
    eig, r = np.linalg.eigh(dm_env)
    r = np.concatenate([np.zeros((nfrag, r.shape[1])), r], axis=0)  # (site|env)
    c_dmet = r[:, np.logical_and(eig > tol, eig < 1-tol)]  # (site|dmet)
    c_occenv = r[:, eig > 1-tol]  # (site|occenv)
    c_virenv = r[:, eig < tol]  # (site|virenv)
    ...
    def build_part(c_partenv):
        ...
        r, sv, orders = recursive_block_svd(fock_part, n=nfrag, tol=tol, maxblock=nmom_max_bath)  # (partenv|partenv)
        c_ewdmet = np.dot(c_partenv, r)  # (site|ewdmet)
        return c_ewdmet
    ...
    print("Combined bath: nbath = {}".format(c_bath.shape[1]))
    ...
    ea = e[nelec//2]
    gap = ip + ea
    print("E(tot) = {:.8f} (ΔE = {:.8f})".format(e_tot, e_tot - e_prev))
